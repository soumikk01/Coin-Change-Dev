...
async function initDB() {
  ...
  try {
    db.run('ALTER TABLE users ADD COLUMN provider TEXT DEFAULT "email"');
    console.log('✅ Migration: Added provider column to users table');
  } catch (e) {
  ...
  const adminResult = db.exec(
    "SELECT id FROM users WHERE email = 'admin@coinchanger.com'"
  );
...
const userDB = {
  // Create new user
  create: (name, email, hashedPassword, provider = 'email') => {
    db.run(
...
const calculationsDB = {
  ...
  getAll: () => {
    const result = db.exec(
      'SELECT id, user_id, user_name, user_email, amount, coins, result, min_coins, created_at FROM calculations ORDER BY created_at DESC'
    );
    if (result.length === 0) return [];
    return result[0].values.map((row) => ({
      id: row[0],
  ...
  getByUser: (userId) => {
    const result = db.exec(
      'SELECT id, amount, coins, result, min_coins, created_at FROM calculations WHERE user_id = ? ORDER BY created_at DESC',
      [userId]
    );
    if (result.length === 0) return [];
    return result[0].values.map((row) => ({
